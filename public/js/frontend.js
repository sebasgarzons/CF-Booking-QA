...
document.querySelectorAll('.add-to-cart-btn-front').forEach(button => {
    button.addEventListener('click', async (e) => {
        ...
        if (authStatus.authenticated) {
            ...
            fetch('https://polar-mountain-17270-cc22e4a69974.herokuapp.com/carrito/add', {
                method: 'POST',
    ...
    if (e.target.classList.contains('add-to-cart-btn')) {
        ...
        try {
            ...
            if (authStatus.authenticated) {
                ...
                // Usuario autenticado: enviar al backend
                const response = await fetch('https://polar-mountain-17270-cc22e4a69974.herokuapp.com/carrito/add', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    credentials: 'include',
                    body: JSON.stringify({ packageId }),
...
document.getElementById('filter-button').addEventListener('click', async () => {
    ...
    try {
        const response = await fetch(`https://polar-mountain-17270-cc22e4a69974.herokuapp.com/packages/filter?${queryParams.toString()}`);
        const data = await response.json();
